	...
	<table class="table" id="shipping_table">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<span class="shipping_number_show">{if $line.url && $line.tracking_number}<a class="_blank" href="{$line.url|replace:'@':$line.tracking_number}">{$line.tracking_number}</a>{else}<a class="_blank" href="http://www.dhl.fr/publish/fr/fr/eshipping/track.high.html?pageToInclude=RESULTS&type=trackindex&brand=I&AWB={$line.tracking_number}">{$line.tracking_number}</a>{/if}</span>
				</td>
				<td>
					{if $line.can_edit}
