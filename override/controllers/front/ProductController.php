    ...
    protected function assignAttributesGroups()
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
				...
				$currentProductIdp = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT idp FROM `sundev_product_attribute` WHERE id_product_attribute = "'.$row['id_product_attribute'].'"');
				$currentProductIdp = isset($currentProductIdp['idp'])?floatval($currentProductIdp['idp']):0;
				$combinations[$row['id_product_attribute']]['idp'] = $currentProductIdp;
				...
				$currentProductCheckPriceUpdate = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT deactivate_price_update FROM `sundev_product_attribute` WHERE id_product_attribute = "'.$row['id_product_attribute'].'"');
                $combinations[$row['id_product_attribute']]['price_update'] = !((bool)$currentProductCheckPriceUpdate);
