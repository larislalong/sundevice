<script type="text/javascript">
	...
	function ProductLteManager(parentBlock){
		...
		this.processGetEditionContent = function () {
			self.clearNotify();
			self.showLoader("{l s='Loading form...'}");
			$.ajax({
				url: "{$link->getAdminLink('AdminProducts', true)|addslashes}",
				dataType: 'json',
				data: {
					ajax: true,
				...
				error: function () {
					self.hideLoader();
					displayErrorOnDiv(self.divProductLTENotify, ["{l s='An error occured while connecting to server'}"]);
				},
				type: 'post',
			});
		};
		...
		this.onSave = function (hasMce) {
			...
			self.showLoader("{l s='Saving data...'}");
			$.ajax({
				url: "{$link->getAdminLink('AdminProducts', true)|addslashes}",
				dataType: 'json',
				data: values,
