<?php /* Smarty version Smarty-3.1.19, created on 2019-01-31 15:49:13
         compiled from "/home/sundevice/public_html/override/controllers/admin/templates/products/combinations.tpl" */ ?>
<?php /*%%SmartyHeaderCode:10554693575c530ae9e30cb6-87439725%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'aa945b829b81bcf07481c318b1aa5ef406e2dfe4' => 
    array (
      0 => '/home/sundevice/public_html/override/controllers/admin/templates/products/combinations.tpl',
      1 => 1543916603,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '10554693575c530ae9e30cb6-87439725',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_5c530aea018353_22186906',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5c530aea018353_22186906')) {function content_5c530aea018353_22186906($_smarty_tpl) {?>
...
<?php }?>
		...
		<div class="form-group row">
			...
			<div class="col-lg-12">
				<div class="checkbox alert alert-warning text-center">
					<label class="control-label" for="deactivate_price_update" >
						<?php echo smartyTranslate(array('s'=>'Désactiver la mise à jour automatique du prix de cette déclinaison ?'),$_smarty_tpl);?>

					</label>
					<label class="control-label" for="deactivate_price_update_on" >
						<input type="radio" name="deactivate_price_update" id="deactivate_price_update_on" <?php if ($_smarty_tpl->tpl_vars['product']->value->deactivate_price_update) {?>checked="checked"<?php }?> value="1" />
						<?php echo smartyTranslate(array('s'=>'OUI'),$_smarty_tpl);?>

					</label>
					<label class="control-label" for="deactivate_price_update_off" >
						<input type="radio" name="deactivate_price_update" id="deactivate_price_update_off" <?php if (!$_smarty_tpl->tpl_vars['product']->value->deactivate_price_update) {?>checked="checked"<?php }?> value="0" />
						<?php echo smartyTranslate(array('s'=>'NON'),$_smarty_tpl);?>
...
<script type="text/javascript">
	var DISABLED_TEXT = '<?php echo smartyTranslate(array('s'=>'Disabled'),$_smarty_tpl);?>
';
	var ENABLED_TEXT = '<?php echo smartyTranslate(array('s'=>'Enabled'),$_smarty_tpl);?>
';
	$("#product_form").delegate("#product-combinations .td_combination_status a", "click", function(event){
		event.preventDefault();
		processCombinationStatusChange($(this));
	});
	function processCombinationStatusChange(target){
		showCombinationStatusChangeLoader(target);
		var actionUrl = target.attr("href")+"&ajax=1&action=CombinationStatusChange";
		$.ajax({
			url: target.attr("href")+"&ajax=1&action=CombinationStatusChange",
			dataType: 'json',
			success: function (result) {
				hideCombinationStatusChangeLoader(target);
				if(!result.hasError){
					var newText = result.active ? ENABLED_TEXT : DISABLED_TEXT;
					var newClass = result.active ? "action-enabled" : "action-disabled";
					var newVisible = result.active ? "icon-check" : "icon-remove";
					
					target.find("i").addClass("hidden");
					target.attr("title", newText);
					target.removeClass("action-enabled").removeClass("action-disabled").addClass(newClass);
					target.find("i."+newVisible).removeClass("hidden");
				}
			},
			error: function () {
				hideCombinationStatusChangeLoader(target);
	...
	function showCombinationStatusChangeLoader(target){
		target.find("i").addClass("hidden");
		var loadingIcon = target.find("pa_icon_loading");
		if(loadingIcon.length > 0){
			loadingIcon.removeClass("hidden");
		}else{
			target.append('<i class="icon-refresh icon-spin pa_icon_loading"></i>');
		}
	}
	function hideCombinationStatusChangeLoader(target){
		target.find("pa_icon_loading").addClass("hidden");
	}
</script>
<?php }?>
<?php }} ?>
