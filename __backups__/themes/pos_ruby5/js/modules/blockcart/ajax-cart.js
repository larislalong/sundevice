...
var newOverrideAjaxCartAdd = function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
	// alert('dev test');
	...
	if($("#input_add_packaging").length && $("#input_add_packaging").is(':checked')){
		defaultData += '&product_add_pack=1';
	}else{
		defaultData += '&product_add_pack=2';
	}
	$.ajax({
		type: 'POST',
		headers: { "cache-control": "no-cache" },
		url: baseUri + '?rand=test-' + new Date().getTime(),
		async: true,
		cache: false,
		dataType : "json",
...
var ajaxCart = {
	...
	overrideButtonsInThePage : function(){
		...
		if ($('.cart_block').length) {
			$(document).off('click', '#add_to_cart button').on('click', '#add_to_cart button', function(e){
				e.preventDefault();
				newOverrideAjaxCartAdd($('#product_page_product_id').val(), $('#idCombination').val(), true, null, $('#quantity_wanted').val(), null);
			});
		}
	...
	add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
		...
		*/
		alert('kjkhjkhjk');
		newOverrideAjaxCartAdd(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist);
			...
			if (this.id != undefined)
			{
				...
				if ($('dt[data-id="cart_block_product_' + domIdProduct + '"]').length == 0)
				{
					...
					// if (typeof(freeProductTranslation) != 'undefined')
						...
						content += '<span class="price">' + (parseFloat(this.price_float) > 0 ? this.priceByLine : 0) + '</span>';
					// content += '</div></div>';
