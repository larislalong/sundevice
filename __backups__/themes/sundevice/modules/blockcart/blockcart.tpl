...
<div class="shopping_cart_header {if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
	<div class="shopping_cart">
		<a class="current"  href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			{*<i class="icon_bag_alt"></i>*}
			<span class="ajax_cart_quantity"><span>{$cart_qties}</span></span>
		</a>
		{if !$PS_CATALOG_MODE}
			<div class="cart_block toogle_content exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
						...
						{/if}
						<p class="cart_block_no_products fontcustom1{if $products} unvisible{/if}">
						...
						<div class="cart-prices">
							...
							<div class="cart-prices-line fontcustom1 last-line">
								...
								<span class="uppercase">{l s='Total' mod='blockcart'}</span>
							</div>
...
{assign var=active_overlay value=1}
...
{addJsDef PS_CART_LIFE_TIME=$PS_CART_LIFE_TIME|intval}
{addJsDef PS_CART_REFRESH_SHOW_TIME=$PS_CART_REFRESH_SHOW_TIME|intval}
{addJsDef cartTotalProduct=$cart_qties|intval}
{addJsDef cartLastUpdate=strtotime($cart->date_upd)}
{addJsDef currentDateTime=strtotime($currentDate)}
{addJsDef isolatedQuantities=$isolatedQuantities}

{addJsDefL name='DAY_TIMER'}{l s='d' mod='blockcart' js=1}{/addJsDefL}
{addJsDefL name='HOURS_TIMER'}{l s=':' mod='blockcart' js=1}{/addJsDefL}
{addJsDefL name='MINUTE_TIMER'}{l s=':' mod='blockcart' js=1}{/addJsDefL}
{addJsDefL name='SECOND_TIMER'}{l s='s' mod='blockcart' js=1}{/addJsDefL}
