...
class BlcAttributesQuery extends ObjectModel
{
	...
	private static function getProductAttributes($selectedFilters, $id_lang, $idShop)
    {
        if (!Combination::isFeatureActive()) {
            return array();
        }
		$productRestrict = empty($selectedFilters)?self::getNoFilterRestrict():'';
        $sql = 'SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, agl.`public_name` AS public_group_name,
					a.`id_attribute`, al.`name` AS attribute_name, a.`color` AS attribute_color, product_attribute_shop.`id_product_attribute`,
					IFNULL(stock.quantity, 0) as quantity, product_attribute_shop.`price`, product_attribute_shop.`ecotax`, product_attribute_shop.`weight`,
					product_attribute_shop.`default_on`, pa.`reference`, product_attribute_shop.`unit_price_impact`,
					product_attribute_shop.`minimal_quantity`, product_attribute_shop.`available_date`, ag.`group_type`, p.*, pl.*,
				...
				WHERE 1 AND al.`id_lang` = '.(int)$id_lang.'
	...
	private static function getNoFilterRestrict()
    {
		...
		return ' AND (pa.id_product_attribute IN ('.implode(',', $idsAttributes).'))';
	}
	...
	public static function addAttributesFilter($values){
		...
		$sql = ' AND (pa.id_product_attribute IN ('.implode(',',$idProductAttributes).')) ';
        return $sql;
	}
	
	public static function getCombinations($selectedFilters, $id_lang, $idShop, $start, $limit, $orderWay, $orderColumnType, $orderColumn, $getFilterLeft = false, $triggerBlockId = 0, $triggerBlockValues = array())
    {
		$combinations = self::getProductAttributes($selectedFilters, $id_lang, $idShop);
		...
		$result = array('hasMore'=>$sorter->hasMore, 'combinations'=>$list, 'groups'=>$combinationsData['groups']);
		if($getFilterLeft){
			$listBlock = empty($selectedFilters) ? null : BlcFilterQuery::getFilterList($combinationsData, $id_lang, $triggerBlockId, $triggerBlockValues, $selectedFilters);
	...
	public static function formatCombinations($list, $id_lang, $start, $limit)
    {
        ...
        foreach($list as $k => $combination){
			...
			if(!isset($combinations[$key])){
				...
				$combinations[$key]['allow_oosp'] = Product::isAvailableWhenOutOfStock($product->out_of_stock);
