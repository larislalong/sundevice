...
class MailAlert extends ObjectModel
{
	...
	public static function getMailAlerts($id_customer, $id_lang, Shop $shop = null)
	{
		...
		for ($i = 0; $i < $products_number; ++$i)
		{
			...
			// if (Tools::getValue('devCheck')){
				// MailAlert::sendCustomerAlert((int)$products[$i]['id_product'], (int)$products[0]['id_product_attribute']);
			// }
				...
				Validate::isUnsignedInt($products[$i]['id_product_attribute']))
			{
				...
				foreach ($attrgrps as $attrgrp)
					...
					{
						$products[$i]['cover'] = $obj->id.'-'.($images[0]);
						break;
					}
			}
	...
	public static function sendCustomerAlert($id_product, $id_product_attribute)
	{
		...
		foreach ($customers as $customer)
		{
			...
			$attributes = self::getProductAttributeCombination($id_product_attribute, $id_lang);
			$attributes_str = '';
			if(isset($attributes) and !empty($attributes)){
				foreach($attributes as $attr){
					$attributes_str .= ', '.$attr['attribute_name'];
				}
			}
