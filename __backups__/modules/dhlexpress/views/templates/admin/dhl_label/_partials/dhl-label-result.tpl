...
{else}
  ...
  <div id="dhl-document-download">
    <p><b>{l s='AWB. number: ' mod='dhlexpress'}</b><span id="dhl-label-awbnumber">{$labelDetails['AirwayBillNumber']|escape:'html':'utf-8'}</span></p>
    <p><b>{l s='DHL product chosen: ' mod='dhlexpress'}</b><span id="dhl-label-product-chosen">{$labelDetails['ProductShortName']|escape:'html':'utf-8'}</span></p>
    <div class="dhl-picto-div">
      <p>
        <a class="dhl-download-label-link"
           target="_blank"
           {if $freeLabel}
             {if $labelDetails['LabelImage']['OutputFormat'] == 'ZPL2'}
               download="{$labelDetails['AirwayBillNumber']|escape:'html':'utf-8'}.zpl"
             {elseif $labelDetails['LabelImage']['OutputFormat'] == 'EPL2'}
               download="{$labelDetails['AirwayBillNumber']|escape:'html':'utf-8'}.epl"
             {else}
               download="{$labelDetails['AirwayBillNumber']|escape:'html':'utf-8'}.pdf"
             {/if}
             href="data:application/octet-stream;base64,{$labelDetails['LabelImage']['OutputImage']|escape:'html':'utf-8'}"
           {else}
