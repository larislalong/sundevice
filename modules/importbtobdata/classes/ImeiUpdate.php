...
class ImeiUpdate
{
    public static function updateAllImei($delivered = false)
    {
		//$deliveredState = Configuration::get('PS_OS_DELIVERED'); (int) $deliveredState . ' ' . ($delivered ? 'IN' : 'NOT IN') .
        $sql = 'SELECT o.id_order, o.reference FROM ' . _DB_PREFIX_ .'orders o WHERE checkimei=0';
	...
	public static function hasDeliveredState($id_order)
    {
		$deliveredState = Configuration::get('PS_OS_DELIVERED');
        $sql = 'SELECT count(*) FROM ' . _DB_PREFIX_ .'order_history WHERE (id_order = ' . (int) $id_order . ') AND (id_order_state = ' . (int) $deliveredState . ')';
		...
		if(!empty($result)){
            ...
            if (isset($data->imeis) && is_array($data->imeis)){
                ...
                foreach ($tabkidp as $key => $value) {
                    ...
                    $testup = Db::getInstance()->update('order_detail', array('emei' => pSQL(implode('<br/>', $value), true)), $where);
                }
                if($testup){
                    $where = ' (id_order = '. (int) $id_order .')';
                    $testup = Db::getInstance()->update('orders', array('checkimei' => 1), $where);
                }
                /*
                if($changeState){
					Configuration::updateValue('PS_ORDER_STATE_AUTO_CHANGE', 1);
