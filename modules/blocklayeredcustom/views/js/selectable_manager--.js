...
function SelectableManager() {
	...
	this.addSelecteds = function (target, selecteds, process) {//=true
	...
	this.setSelecteds = function (idBlock, blockType, selecteds, process) {//=true
	...
	this.resetFilter = function (process) {//=true
		self.filterString = "";
		if(process){
			self.processFilter();
		}
    };
	this.processFilter = function (idBlock) {// = 0
...
function SelectableDropdownList(selectableManager, parentDiv) {
    ...
    this.handleEvent = function () {
		parentDiv.delegate(self.selector, "change", function (event) {
    		var target = $(this);
			selectableManager.addSelecteds(target, self.getSelecteds());
    	});
    };
